...
**We are building this for:** Embedded Rust developers building WS2812-based LED projects on ESP32 or AVR — the two first-class supported MCU families — who want testable, composable building blocks rather than monolithic driver crates.

**Long-term goals:**
- Animation vocabulary on demand — ring effects users find without forking
- Complete `no_std` / embassy support via `rustyfarian-esp-hal-ws2812`
- Ecosystem currency across ESP32 and AVR — timely adoption of new chip variants, HAL releases, and toolchain shifts

**Out of scope:** Application code, demo or networking projects (e.g. ESP-NOW demos live in separate workspaces), exhaustive pre-built animation catalogues, matrix-first effect vocabulary, and anything that doesn't serve the embedded WS2812 use case.
...
See [Why Yet Another WS2812 Crate?](docs/why-yet-another-ws2812-crate.md) for the full design rationale, and [docs/blinksy-ecosystem-evaluation.md](docs/blinksy-ecosystem-evaluation.md) for the side-by-side comparison.
...
| [`bunting`](crates/bunting)                                       | Pure Rust WS2812 utilities (color conversion, bit encoding); also provides `grid` — pure data-layout types for matrix displays                          | `no_std` compatible | [![crates.io version](https://img.shields.io/crates/v/bunting.svg)](https://crates.io/crates/bunting)                                       | [![docs.rs](https://img.shields.io/docsrs/bunting)](https://docs.rs/bunting)                                       |
| [`rustyfarian-esp-idf-ws2812`](crates/rustyfarian-esp-idf-ws2812) | WS2812 driver using ESP-IDF RMT peripheral                                                                                                              | ESP-IDF (std)       | [![crates.io version](https://img.shields.io/crates/v/rustyfarian-esp-idf-ws2812.svg)](https://crates.io/crates/rustyfarian-esp-idf-ws2812) | [![readme](https://img.shields.io/badge/docs-readme-blue)](crates/rustyfarian-esp-idf-ws2812/README.md)            |
...
| [`rustyfarian-avr-ws2812`](crates/rustyfarian-avr-ws2812)         | WS2812 driver for AVR — cycle-counted bit-bang and SPI prerendered backends (`embedded-hal` 1.0)                                                        | AVR (no_std)        | [![crates.io version](https://img.shields.io/crates/v/rustyfarian-avr-ws2812.svg)](https://crates.io/crates/rustyfarian-avr-ws2812)         | [![docs.rs](https://img.shields.io/docsrs/rustyfarian-avr-ws2812)](https://docs.rs/rustyfarian-avr-ws2812)         |

> **Note:** For `rustyfarian-avr-ws2812`, the cycle-counted bit-bang backend (feature `bitbang`) is recommended; the SPI backend is an additional option (see [ADR 007](docs/adr/007-avr-ws2812-driver-strategy.md)).

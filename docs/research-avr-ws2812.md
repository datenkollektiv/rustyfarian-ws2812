...
- `avr-hal` completed its `embedded-hal 1.0` migration in January 2025 (issue #77 closed), but `ws2812-spi` still targets `embedded-hal 0.2` — the two ecosystems are currently mismatched.
...
(`llvm-project#118015` and `llvm-project#121498`) that caused out-of-range branch targets
on devices lacking the `jmp` instruction (notably ATtiny85).
These fixes landed in LLVM 20 and were integrated via `rust-lang/rust#135763`.

A separate open issue (`rust-lang/rust#134758`, December 2024) affects `global_asm!` in library
...
The `global_asm!` bug described above (`rust-lang/rust#134758`) can affect this crate when
...
Issue #77 ("Switch to embedded-hal 1.0.0") was **closed as completed on 4 January 2025**.
The migration was spread across multiple PRs; PR #488 did the bulk of the work, and PR #621
...
The timing-critical inner loop is written in **AVR assembly language** with extensive C pseudocode
...
`rust-lang/rust#134758` (open as of January 2025) prevents `global_asm!` in library crates

...
Some applications do not have a physical status LED,
but crates like `rustyfarian-network` require the type parameter.
A `NoLed` stub satisfies the trait without a hardware dependency.

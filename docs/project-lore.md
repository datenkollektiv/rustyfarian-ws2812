...
Smoking-gun diagnostic: with `NUM_LEDS = 1`, downstream LEDs in the chain still flicker — proof that LED 1 is not consuming exactly 24 bits, so partial/extra bits leak via the pass-through. Rules in encoding-level bit-counting unreliability; rules out colour-only theories (GRB order, PulseEffect math, brightness scaling). Crystal mismatch, power supply, cable length, and clone-vs-genuine board were also ruled out (2026-05-04).

...
- `rustyfarian-esp-idf-ws2812`: `idf_esp32_rgb_cycle` example — drives a discrete RGB LED via `pennant`'s `RgbGpioLed`, cycling the eight on/off colours for on-hardware verification (uses the Cheap Yellow Display's RGB GPIO numbers 4/16/17; defaults to an external common-cathode LED, `ActiveHigh` — set `ActiveLow` for the CYD's common-anode onboard LED); build with `just build-example-esp32-rgb`, flash with `just run idf_esp32_rgb_cycle`. Gated behind the new example-only `rgb-gpio` feature (`pennant/hal`)
- `rustyfarian-esp-idf-ws2812`: `idf_esp32_rgb_pulse` example — fades a discrete RGB LED in and out with `PulseEffect` via `pennant`'s `RgbPwmLed`, driven by three ESP-IDF LEDC PWM channels so the effect renders as a smooth brightness fade rather than a blink (Cheap Yellow Display GPIO 4/16/17, common-cathode/active-high by default); build with `just build-example-esp32-rgb-pulse`, flash with `just run idf_esp32_rgb_pulse`. Gated behind the new example-only `rgb-pwm` feature (`pennant/hal`)
...
- `rustyfarian-esp-hal-ws2812`: the bump is hardware-validated on an ESP32-C6 across the blocking, async, Embassy multi-task, `smart-leds` and GPIO8 onboard paths, and on an ESP32-C3 via `hal_c3_pulse` — the latter closing a hardware-validation gap open since 2026-04-29. ESP32-WROOM-32 (Xtensa) remains compile-verified only. See [`docs/features/esp-hal-stack-upgrade-august-2026-v1.md`](docs/features/esp-hal-stack-upgrade-august-2026-v1.md)

...
- `rustyfarian-esp-hal-ws2812`: coordinated bump of the exact-pinned bare-metal stack — `esp-hal` `=1.1.2` → `=1.2.2`, `esp-rtos` `=0.3.0` → `=0.4.0`, `esp-bootloader-esp-idf` `=0.5.0` → `=0.6.0`, `esp-println` `=0.17.0` → `=0.18.0`. The Embassy pins (`embassy-executor 0.10.0`, `embassy-sync 0.8.0`, `embassy-time 0.5.1`) are unchanged; `esp-rtos 0.4` still requires the first two and does not depend on `embassy-time` at all. The payload is in `esp-rtos 0.4.0`: a potential RISC-V crash fix (#5641) and task-stack lifetime fixes (#6032, #6027). `esp-hal 1.2.x` changes nothing on the RMT/GPIO path this driver uses. Compile-verified on ESP32-C6, ESP32-C3 and Xtensa ESP32, and **hardware-validated on 2026-09-24** across the blocking, async, Embassy multi-task, `smart-leds` and GPIO8 onboard paths on an ESP32-C6, plus blocking and async on an ESP32-C3; ESP32-WROOM-32 (Xtensa) remains compile-verified only — see [`docs/features/esp-hal-stack-upgrade-september-2026-v1.md`](docs/features/esp-hal-stack-upgrade-september-2026-v1.md)
- `rustyfarian-esp-hal-ws2812`: all five async examples (`hal_c3_pulse_async`, `hal_c6_pulse_async`, `hal_c6_rainbow_comet_async`, `hal_c6_multitask_async`, `hal_esp32_pulse_async`) migrated to `esp_rtos::start(timg0.timer0, peripherals.FROM_CPU_INTR0)`. `esp-hal 1.2.0` removed `SoftwareInterruptControl` and the `SW_INTERRUPT` singleton (#6142); the call is now identical on RISC-V and Xtensa, so the Xtensa example's one-argument form and its explanatory comment are gone
- MSRV: `rustyfarian-esp-hal-ws2812` and `rustyfarian-esp-idf-ws2812` now declare `rust-version = "1.95"` (required by `esp-hal 1.2` / `esp-rtos 0.4`; the IDF crate follows by policy). The workspace floor stays at `1.88` for `bunting`, `pennant`, `ferriswheel` and `rustyfarian-avr-ws2812`: the AVR driver builds on the pinned `nightly-2025-04-27` (matching upstream `avr-hal`), which reports 1.88, and Cargo refuses any package in the graph — path dependencies such as `bunting` included — whose `rust-version` exceeds the active toolchain. A workspace-wide 1.95 was tried and failed `just check-avr-target` on exactly that error. README, `AGENTS.md` and the AVR crate README document the split
- `deny.toml`: RUSTSEC-2024-0436 (`paste`) rationale re-cited to `esp-hal 1.2.2`, which still declares `paste` as a direct dependency; the suppression itself is unchanged
- `docs/esp-hal-version-matrix.md`: new "Current State" section for the September 2026 wave; the 1.1.2 section moves to historical
- `rustyfarian-esp-idf-ws2812`: bumped `esp-idf-hal` from `0.46` to `0.47` (resolves `esp-idf-hal 0.47.0` / `esp-idf-sys 0.38.1`; 0.38.0 is yanked upstream). Relevant upstream fixes: `PinDriver` now routes the pad to the GPIO function before setting its direction (#585), the path used by the discrete-RGB examples on CYD GPIO 4/16/17; async SPI hangs on multi-core chips; basic ESP-IDF 6.0 compatibility. MSRV of the IDF stack is unchanged at 1.82. **The `send_and_wait` workaround remains required**: 0.47.0's `EncoderWrapper` still panics on bitwise-OR'd `rmt_encode_state_t` flags, and no async RMT transmit was added (only an async `wait_for_progress`). `esp-idf-sys 0.38` adds a build-time `libc` symbol-compatibility check; the workspace's locked `libc` passed it
- `rustyfarian-esp-idf-ws2812`: bumped the pinned `embuild` build-dependency from `=0.33.1` to `=0.33.5` in two patch steps (`=0.33.3` on 2026-08-12, `=0.33.5` on 2026-09-21; build-time only, no runtime footprint). 0.33.4 restores `cargo::workspace_dir` detection under the build-script output layout introduced by Cargo 1.100 nightlies; 0.33.5 adds the optional `elfsize` firmware-size reporting, unused here. 0.33.5 is also the floor required by `esp-idf-hal 0.47`
- CI: bumped the three actions still running on the deprecated Node 20 runtime — `actions/checkout` `v4` → `v5`, `extractions/setup-just` `v2` → `v4`, and `actions/cache` `v4` → `v6` (the last used only by the Xtensa toolchain cache). GitHub removed Node 20 from its runners on 2026-09-23 and had been force-running these on Node 24; all three target tags declare `runs.using: node24`, verified against each action's `action.yml`. `setup-just` v4 is now a composite action delegating to `extractions/setup-crate` pinned by commit SHA, with the `just-version` / `github-token` input surface unchanged — and no workflow here sets either. `Swatinem/rust-cache@v2`, `taiki-e/install-action@v2`, `dtolnay/rust-toolchain` and `esp-rs/xtensa-toolchain@v1.7.0` needed no change: the first already resolves to a node24 build on its floating `v2` tag, and the other three are composite actions the deprecation never applied to
